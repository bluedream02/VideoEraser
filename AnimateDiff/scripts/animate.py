...
# from video.AnimateDiff.animatediff.pipelines.pipeline_animation_rebuttal_np import AnimationPipeline
    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for model_idx, model_config in enumerate(config):
        ...
        if args.erased_concept:
            copyright_concepts = [args.erased_concept]
        else:
            copyright_concepts = list(model_config.copyright_concept) * len(prompts) if len(model_config.copyright_concept) == 1 else model_config.copyright_concept
        ...
        for prompt_idx, (prompt, n_prompt, random_seed, copyright_concept) in enumerate(zip(prompts, n_prompts, random_seeds, copyright_concepts)):
            ...
            sample = pipeline(
                ...
                safety_concept=copyright_concept,
            ).videos
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="VideoEraser: Generate videos with concept erasure using AnimateDiff"
    )
    
    # Model and config
    parser.add_argument("--pretrained-model-path", type=str, 
                        default="models/stable-diffusion-v1-5",
                        help="Path to pretrained Stable Diffusion model (relative or HuggingFace model ID)")
    parser.add_argument("--inference-config", type=str, 
                        default="configs/inference/inference-v1.yaml",
                        help="Path to inference config")    
    parser.add_argument("--config", type=str, default="configs/prompts/example.yaml",
    ...
    parser.add_argument("--erased-concept", type=str, default=None,
                        help="Concept to erase from generation (overrides config)")
    parser.add_argument("--seed", type=int, default=None,
                        help="Random seed for reproducibility (overrides config)")
    
    # Output options
    parser.add_argument("--output-dir", type=str, default=None,
                        help="Output directory for generated videos (default: samples/{config}-{timestamp})")
